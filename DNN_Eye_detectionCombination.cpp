...
void detectionfunction(Net net, Mat &frameOpenCVDNN)
{
	...
	eyeCascade.detectMultiScale(frameOpenCVDNN, eyes);
	...
	for (int i = 0; i < detectionMat.rows; i++)
	{
		...
		if (confidence > confidenceThreshold)
		{
			...
			for (int j = 0; j < eyes.size(); j++) {
				if (x1 < eyes[j].x && y1 < eyes[j].y && x1 + x2 > eyes[j].x + eyes[j].width && y1 + y2 > eyes[j].y + eyes[j].height) {					
...
int main(int argc)
{
	...
	//load the HAAR eye cascade model file
	eyeCascadePath = "./haarcascade_eyed.xml";
